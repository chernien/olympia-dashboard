...
export interface CartItemState extends EntityState<any> {
    data: any[]
}
...
export function cartReducer(state = getState('molla'), action:any) {
    switch (action.type) {
        case ADD_TO_CART:
            var findIndex = state.data.findIndex((item:any) => item.id == action.payload.product.id);
            let qty = action.payload.qty ? action.payload.qty : 1;
            if (findIndex !== -1 && action.payload.product.variants.length > 0) {
                findIndex = state.data.findIndex((item:any) => item.name == action.payload.product.name);
            }

            if (findIndex !== -1) {
                return {
                    data: [
                        ...state.data.reduce((acc:any, product:any, index:any) => {
                            if (findIndex == index) {
                                acc.push({
                                    ...product,
                                    qty: product.qty + qty,
